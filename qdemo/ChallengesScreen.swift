...
struct ChallengesScreen: View {
  ...
  @State private var showGimmeeScreen = false
  var body: some View {
    ZStack {
      DismissButtonView()
      ...
      VStack {
        ...
        // some bottomishbuttons
        Spacer()
        HStack{
          Button("Gimme") {
            print("Gimmee")
            showGimmeeScreen = true
          }.disabled(gameState.gimmees==0)
            .opacity(gameState.gimmees==0 ? 0.1 : 1.0)
          Spacer()
          Button("Pass") {
            print("Pass")
            dismiss()
    ...
    .sheet(isPresented: $showGimmeeScreen){
