...
struct MatrixItemView: View {
  init(number: Int, settings: AppSettings, onTap: ((Int) -> Void)? = nil, onLongPress: ((Int) -> Void)? = nil,shownum:Bool = false) {
    self.number = number
  ...
  var body: some View {
    ...
    Text(shownum ? "\(number)" : challenges[number].question) // replace with "\(number)" for gpt
...
#Preview {
  MatrixItemView(number: 1, settings: AppSettings(),shownum:true)
}
#Preview {
  MatrixItemView(number: 1, settings: AppSettings(),shownum:true)
